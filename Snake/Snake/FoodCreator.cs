...
namespace Snake
{
    class FoodCreator //класс для генерации точек, соответствующих "еде"
    {
        ...
        /*public - ключевое слово, которое помечает методы и переменные, вызываемые за пределами класса
        без указанного public неявно предполагается наличие ключевого слова private (методы и переменные невидимы за пределами класса)
        сокрытие - ключевое проявление принципа инкапсуляции*/
        /*наследование позволяет создавать новые классы, которые повторно используют, расширяют, изменяют поведение, определенное у других классов
        (базовые классы + наследники)*/
    }
}
